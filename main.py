...
def handle_identifier():
    ...
    print('-' * 40)
    print('请选择一级类别：')
    for item in list(category_dic.keys()):
        print(item, end='\t')
    ...
    print('-' * 40)
    print('请选择二级类别：')
    detail_list = category_dic[list(enumerate(category_dic))[chief_cat - 1][1]]
...
def main_loop():
    ...
    global bookinfo_cursor, proof_cursor
    donor = ''
    ...
    while not quit:
        ...
        category, bookid = handle_identifier()
        pre_donor = donor
        donor = handle_donor()
        comment = handle_comment()

        # Add to bookinfo sheet.
        bookinfo_sheet.range(f'A{bookinfo_cursor}').value = [
            title, author, category, bookid, donor, comment]

        # Beautify the sheet.
        bookinfo_sheet.autofit()
        ...
        if donor == pre_donor:
            ...
            if will_cover == 'y':
                proof_cursor -= 1
                proof_sheet.range(f'B{proof_cursor}').value += f'\n{bookid}'

            # If no, persistence continues.
            else:
                proof_sheet.range(f'A{proof_cursor}').value = [proofid, bookid]
...
wb = app.books.open('./漂流书架.xlsx')

# Process bookinfo sheet.
bookinfo_sheet = wb.sheets[0]
bookinfo_sheet.range('A2').expand().api.Delete()
bookinfo_cursor = 2

# Process proof sheet.
proof_sheet = wb.sheets[1]
proof_cursor = proof_sheet.range('A1').expand('down').last_cell.row + 1
